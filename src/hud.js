...
const BOARD_MIN = 3
const BOARD_ROW = 20
const BOARD_CHROME = 56
const BOARD_GAP = 16
const UI_EDGE = 44
const UI_FLOOR = 0.5
const PROMO_ID = 'promo-banner'
const PROMO_ROOM = 8
const PROMO_TRIES = 3
const PROMO_WAIT = 3000
...
export function createStart(layer, onStart, netApi) {
  ...
  el.innerHTML = `
    <div class="stack">
      ...
      <div class="promo" id="${PROMO_ID}"></div>
    </div>`
  ...
  const promo = el.querySelector('.promo')
  let promoTimer = null
  let promoTries = 0
  const shutPromo = () => {
  ...
  const placePromo = () => {
    const host = promo.offsetParent
    const box = el.querySelector('.box')
    const foot = el.querySelector('.home-footer')
    if (!host || !box || !foot) return false
    const head = box.getBoundingClientRect().bottom
    const tail = foot.getBoundingClientRect().top
    const free = tail - head - promo.offsetHeight
    if (free < PROMO_ROOM) return false
    promo.style.top = Math.round(head + free / 2 - host.getBoundingClientRect().top) + 'px'
    return true
  }
  const onSize = () => {
    if (bannerState() === 'off') return
    promo.style.display = ''
    if (!placePromo()) promo.style.display = 'none'
  }
  window.addEventListener('resize', onSize)
  if (bannerState() === 'off') shutPromo()
  const paintPromo = (wait) => {
    ...
    promoTimer = setTimeout(() => {
      ...
      if (state === 'wait') {
        paintPromo(PROMO_WAIT)
        return
      }
      if (promoTries >= PROMO_TRIES) return
      if (!placePromo()) {
        shutPromo()
        return
      }
      promoTries++
      showBanner(promo).then((ok) => {
        if (ok || !el.isConnected) return
        if (bannerState() === 'off') shutPromo()
        else paintPromo(PROMO_WAIT)
      })
    }, wait)
  }
  const dropPromo = () => {
  ...
  paintPromo(logoSeen ? PROMO_SETTLE : PROMO_INTRO)
...
export function createHud(layer, cb) {
  ...
  fitHud(el)
  setTimeout(() => fitHud(el), 400)
  window.addEventListener('resize', () => fitHud(el))
...
let boardCap = BOARD_MAX
let boardCapAt = 0

let uiFit = 1

function uiScale() {
  return document.body.classList.contains('touch') ? 1 : uiFit
}

function fitHud(el) {
  const panel = el.querySelector('.skills')
  const tall = panel ? panel.offsetHeight : 0
  if (!tall) return
...
function boardRows(r) {
  ...
  if (tall === boardCapAt) return boardCap
  const panel = r.upgradeId ? r.upgradeId.getBoundingClientRect().height : 0
  if (!panel) return boardCap
  boardCapAt = tall
  const ui = uiScale()
  const room = tall - panel - BOARD_CHROME * ui - BOARD_GAP
  boardCap = Math.max(BOARD_MIN, Math.min(BOARD_MAX, Math.floor(room / (BOARD_ROW * ui))))
  return boardCap
}
