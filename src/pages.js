...
const OUTLINKS = import.meta.env.MODE !== 'gd'
const LINK = /<a[^>]*>([\s\S]*?)<\/a>/g
  ...
  en: {
    privacy: {
      ...
      body: `
        ...
        <p>The game itself sets no advertising cookie and builds no player profile. The portals that host it do show their own advertising and set their own cookies, which are outside my control. On those sites the game loads the portal software kit, which alone handles the advertising, and the privacy policy of the host portal applies on top of this one. It is reachable from the site you are playing on.</p>
        <h4>7. Children</h4>
  ...
  es: {
    privacy: {
      ...
      body: `
        ...
        <p>El juego en sí no pone ninguna cookie publicitaria ni crea ningún perfil de jugador. En cambio, los portales que lo alojan muestran su propia publicidad y ponen sus propias cookies, sobre las que no tengo control. En esos sitios el juego carga el kit de desarrollo del portal, que gestiona por sí solo la publicidad, y se aplica la política de privacidad del portal anfitrión además de esta. Puede consultarse desde el sitio en el que estás jugando.</p>
        <h4>7. Menores</h4>
  ...
  de: {
    privacy: {
      ...
      body: `
        ...
        <p>Das Spiel selbst setzt kein Werbe-Cookie und erstellt kein Spielerprofil. Die Portale, die es hosten, zeigen jedoch eigene Werbung und setzen eigene Cookies, auf die ich keinen Einfluss habe. Auf diesen Seiten lädt das Spiel das Entwicklungskit des Portals, das die Werbung allein steuert, und zusätzlich zu dieser gilt die Datenschutzerklärung des jeweiligen Portals. Sie ist über die Seite erreichbar, auf der Sie spielen.</p>
        <h4>7. Kinder</h4>
  ...
  pt: {
    privacy: {
      ...
      body: `
        ...
        <p>O jogo em si não coloca cookies publicitários nem cria qualquer perfil de jogador. Já os portais que o alojam mostram publicidade própria e colocam os seus próprios cookies, sobre os quais não tenho controlo. Nesses sites o jogo carrega o kit de desenvolvimento do portal, que trata sozinho da publicidade, e aplica-se a política de privacidade do portal anfitrião além desta. Pode ser consultada a partir do site onde está a jogar.</p>
        <h4>7. Crianças</h4>
  ...
  ru: {
    privacy: {
      ...
      body: `
        ...
        <p>Сама игра не ставит рекламных куки и не создаёт профиль игрока. Но порталы, где она размещена, показывают собственную рекламу и ставят собственные куки, которые мне неподконтрольны. На этих сайтах игра загружает набор разработчика портала, который сам управляет показом рекламы, и вдобавок к этой действует политика конфиденциальности портала. Её можно открыть с сайта, на котором вы играете.</p>
        <h4>7. Дети</h4>
  ...
  tr: {
    privacy: {
      ...
      body: `
        ...
        <p>Oyunun kendisi reklam çerezi koymaz ve oyuncu profili oluşturmaz. Ancak oyunu barındıran portallar kendi reklamlarını gösterir ve kendi çerezlerini koyar; bunlar benim denetimimde değildir. Bu sitelerde oyun portalın geliştirici kitini yükler, reklamları tek başına o yönetir ve bu metne ek olarak barındıran portalın gizlilik politikası geçerlidir. Oynadığınız siteden ulaşabilirsiniz.</p>
        <h4>7. Çocuklar</h4>
...
const GUIDE = {
  fr: {
    ...
    rules: 'Les règles',
    r1: `Déplacez-vous avec ZQSD ou les flèches, visez à la souris et tirez au clic gauche ou avec la barre d'espace. Sur écran tactile, le joystick de gauche déplace et le rouge vise et tire. Échap ou P met le jeu en pause, M coupe la musique.`,
  ...
  en: {
    ...
    rules: 'The rules',
    r1: `Move with WASD or the arrow keys, aim with the mouse and fire with the left click or the space bar. On a touch screen the left stick moves and the red one aims and fires. Escape or P pauses the game, M mutes the music.`,
  ...
  es: {
    ...
    rules: 'Las reglas',
    r1: `Muévete con WASD o las flechas, apunta con el ratón y dispara con el clic izquierdo o la barra espaciadora. En pantalla táctil, la palanca izquierda mueve y la roja apunta y dispara. Escape o P pausan el juego, M silencia la música.`,
  ...
  de: {
    ...
    rules: 'Die Regeln',
    r1: `Bewege dich mit WASD oder den Pfeiltasten, ziele mit der Maus und schieße mit der linken Maustaste oder der Leertaste. Auf dem Touchscreen bewegt der linke Stick, der rote zielt und schießt. Escape oder P pausiert das Spiel, M schaltet die Musik stumm.`,
  ...
  pt: {
    ...
    rules: 'As regras',
    r1: `Move-te com WASD ou as setas, aponta com o rato e dispara com o clique esquerdo ou a barra de espaços. No ecrã tátil, o manípulo esquerdo move e o vermelho aponta e dispara. Escape ou P coloca o jogo em pausa, M silencia a música.`,
  ...
  ru: {
    ...
    rules: 'Правила',
    r1: `Двигайтесь на WASD или стрелках, целитесь мышью, стреляете левой кнопкой или пробелом. На сенсорном экране левый джойстик двигает, красный целится и стреляет. Escape или P ставит игру на паузу, M выключает музыку.`,
  ...
  tr: {
    ...
    rules: 'Kurallar',
    r1: `WASD veya yön tuşlarıyla hareket edin, fareyle nişan alın, sol tık veya boşluk tuşuyla ateş edin. Dokunmatik ekranda sol çubuk hareket ettirir, kırmızı olan nişan alıp ateş eder. Escape veya P oyunu duraklatır, M müziği kapatır.`,
...
export function legalPage(lang, kind) {
  const page = pageFor(lang, kind)
  if (OUTLINKS || !page) return page
  return { title: page.title, body: page.body.replace(LINK, '$1') }
}
