...
function readyToShow() {
  if (painted < 1) return false
  if (liveHope && performance.now() < bootAt + LIVE_WINDOW) return false
  return true
}
...
function openWorld() {
  ...
  if (!data) return
  if (net.watching && !game.inGame && !game.player && (shown || performance.now() > bootAt + LIVE_WINDOW)) {
    net.pending = null
    liveHope = false
    dropNetwork()
    return
...
function busiestSpot() {
  ...
  let most = 0
  for (const b of game.world.beings) {
    if (!b.alive || b.first || !b.def) continue
...
async function watchLive() {
  if (!livePeek || game.inGame || net.active) {
    liveHope = false
    return
  }
  ...
  if (!mode || game.inGame || net.active) {
    liveHope = false
    return
  }
  net.watching = true
